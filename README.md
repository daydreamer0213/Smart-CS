...
`v0.1.0` 是 2026-07-21 验收的求职作品集快照：Milestone 1 企业 HR Service Agent 基座和 Milestone 2 文档智能与知识治理均已交付，完整回归为 `403 passed, 4 skipped`。项目当前进入维护冻结，不再主动扩展功能；只处理可复现缺陷、敏感信息和求职展示材料。M3 真实 HR 工具接入与 M4 生产化加固保留为未来选做路线，不是本快照的未完成缺陷。
